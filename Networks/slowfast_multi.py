...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=10, dropout=0.5):
        ...
        self.fc = nn.Linear(self.fast_inplanes + 2048, fc_hd_num, bias=False)

        self.fc1 = nn.Linear(fc_hd_num, class_num)
        self.relu = nn.ReLU()

        self.fc_act = nn.Linear(fc_hd_num, 256)
        self.fc2_act = nn.Linear(fc_hd_num, 256)
    ...
    def forward(self, input):
        ...
        activity_pred = self.fc1(x1)

        activity_output = self.fc2_act(x1)

        clip_feats_int = self.fc_act(x1)
        ...
        # print('clip_feats_int.size()', clip_feats_int.size(), 'activity_pred.size()',activity_pred.size(),
        #       'activity_output.size()', activity_output.size())
...
if __name__ == "__main__":
    num_classes = 101
    input_tensor = torch.autograd.Variable(torch.rand(3, 3, 16, 112, 112))
